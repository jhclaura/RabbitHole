...
function update(){
	...
	//DUMMY
	dummy.position.set(pointerControls.posX(), -2, pointerControls.posZ());
	var originPoint = dummy.position.clone();

	//ALIEN_TVs
	// if(alienTvs.length>0){
		for(var i=0; i<alienTvs.length; i++){
			alienTvs[i].applyBehaviors(alienTvs, new THREE.Vector3(pointerControls.posX(), -2, pointerControls.posZ()));
			alienTvs[i].update();

			alienScrs[i].applyBehaviors(alienScrs, new THREE.Vector3(pointerControls.posX(), -2, pointerControls.posZ()));
			alienScrs[i].update();
		}
	// }
